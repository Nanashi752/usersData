  ...
  return (
    <div className="flex flex-col lg:flex-row justify-between text-white py-10 px-20 border-2 rounded-xl bg-black shadow-lg">
      <div className="w-full lg:w-1/3 bg-zinc-800 p-4 rounded-xl shadow-md">
        {userData.map((item) => (
          <div key={item.id} onClick={() => toggleUserSelection(item.id)} className={`cursor-pointer bg-black p-2 ${selectedUserIds.includes(item.id) ? 'bg-yellow-500' : 'hover:bg-yellow-500 transition duration-200 ease-in-out'} rounded-lg flex items-center space-x-4 mb-4`}>
